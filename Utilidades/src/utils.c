...
t_buffer* serializar_tarea_io(uint32_t tid, uint32_t parametros, char* nombre_tarea) {
	...
	t_tarea_io* tarea = fill_tarea_io(tid, parametros, nombre_tarea);
	t_buffer* buffer = malloc(sizeof(t_buffer));
	buffer->size = sizeof(uint32_t) * 3 + strlen(nombre_tarea) + 1;
	void* stream = malloc(buffer->size);
	int offset = 0;

	memcpy(stream+offset, &(tarea->tid), sizeof(uint32_t));
	offset+=sizeof(uint32_t);
	memcpy(stream+offset, &(tarea->parametros), sizeof(uint32_t));
	offset+=sizeof(uint32_t);
	memcpy(stream+offset, &(tarea->len_string), sizeof(uint32_t));
	offset+=sizeof(uint32_t);
	memcpy(stream+offset, tarea->string, (strlen(tarea->string) + 1));
	buffer->stream = stream;
